   ...
   */
   ...
   switch(switch_action) 
   {
      ...
      case 2: 
         ...
         if (compArray[1] == "1"){
            ret = DoubleToStr(AccountBalance());
            InformPullClient(pSocket, StringFormat("%s", ret));
         }
      ...
      /*
        ...
        */ 
         ...
         for (int index = 0; index < OrdersTotal(); index++){
            ...
            if (OrderSymbol() == compArray[2]){
               if (OrderType() == OP_BUY){
                  ans = OrderClose(OrderTicket(), OrderLots(), MarketInfo(compArray[2], MODE_BID), Slippage, MediumSeaGreen);
               }
               if (OrderType() == OP_SELL){
                  ans = OrderClose(OrderTicket(), OrderLots(), MarketInfo(compArray[2], MODE_ASK), Slippage, DarkOrange);
               }
            }
         }
         
         if (ans){
            InformPullClient(pSocket, "Order closed successfully");
         }
         else{
            InformPullClient(pSocket, "Order close failed");
...
double CalculateLotSize(double SL, string pair){          //Calculate the size of the position size 
   double LotSize=0;
   //We get the value of a tick
   
   //double nTickValue=MarketInfo(pair,MODE_TICKVALUE);
   
   double nTickValue = MarketInfo(pair, MODE_TICKVALUE) / MarketInfo(pair, MODE_TICKSIZE) / MathPow(10.0, MarketInfo(pair, MODE_DIGITS));
   
   /*
   Print("HERE");
   Print(MarketInfo(pair, MODE_TICKSIZE));
   Print(MathPow(10.0, MarketInfo(pair, MODE_DIGITS)));
   Print(nTickValue);
   Print(nnTickValue);
   */
   
   //If the digits are 3 or 5 we normalize multiplying by 10
   if(MarketInfo(pair, MODE_DIGITS)==3 || MarketInfo(pair, MODE_DIGITS)==5){
...
double CalculateNormalizedDigits(string pair)
{
   ...
   else if(MarketInfo(pair, MODE_DIGITS)>=4){
      return(10000);
   }
